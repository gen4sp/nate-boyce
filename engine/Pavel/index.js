...
export default function (canvas) {
  ...
  const baseVertexShader = compileShader(gl.VERTEX_SHADER, SHADERS.baseVert)

  const copyShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.copyFrag)

  const clearShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.clearFrag)

  const colorShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.colorFrag)

  const checkerboardShader = compileShader(
    gl.FRAGMENT_SHADER,
    SHADERS.checkboardFrag
  )

  const splatShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.splatFrag)

  const advectionShader = compileShader(
    gl.FRAGMENT_SHADER,
    SHADERS.advectionFrag,
    ext.supportLinearFiltering ? null : ['MANUAL_FILTERING']
  )

  const divergenceShader = compileShader(
    gl.FRAGMENT_SHADER,
    SHADERS.divergenceFrag
  )

  const curlShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.curlFrag)

  const vorticityShader = compileShader(
    gl.FRAGMENT_SHADER,
    SHADERS.vorticityFrag
  )

  const pressureShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.pressureFrag)

  const gradientSubtractShader = compileShader(
    gl.FRAGMENT_SHADER,
    SHADERS.gradientSubstract
  )
  ...
  const displayMaterial = new Material(baseVertexShader, SHADERS.displayFrag)
